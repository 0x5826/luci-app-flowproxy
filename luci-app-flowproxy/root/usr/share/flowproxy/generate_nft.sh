...
process_rule() {
    local section="$1"; local proto="$2"; local enabled match_type match_value action counter
    enabled=$(uci -q get "$CONFIG.$section.enabled"); [ "$enabled" = "0" ] && return
    match_type=$(uci -q get "$CONFIG.$section.match_type"); match_value=$(uci -q get "$CONFIG.$section.match_value")
    action=$(uci -q get "$CONFIG.$section.action"); [ -z "$action" ] && action="return"; counter=$(uci -q get "$CONFIG.$section.counter")
    [ -z "$match_value" ] && return
    case "$match_value" in @*) local set_ref=$(echo "$match_value" | cut -d' ' -f1); [ "$set_ref" != "@proxy_server_ip" ] && ! echo "$ENABLED_SETS" | grep -q "$set_ref" && return ;; esac
